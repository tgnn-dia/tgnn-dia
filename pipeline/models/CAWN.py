...
class CAWN(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, output_dim: int, position_feat_dim: int, walk_length: int = 2,
                 num_walk_heads: int = 8, dropout: float = 0.1, device: str = 'cpu'):
        """
        ...
        self.walk_encoder = WalkEncoder(
            input_dim=self.node_feat_dim + self.edge_feat_dim + self.time_feat_dim + self.position_feat_dim,
            position_feat_dim=self.position_feat_dim, output_dim=self.output_dim, num_walk_heads=self.num_walk_heads,
            dropout=dropout)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_multi_hop_graphs = self.neighbor_sampler.get_multi_hop_neighbors(num_hops=self.walk_length,
                                                                                  node_ids=src_node_ids,
                                                                                  node_interact_times=node_interact_times,
                                                                                  num_neighbors=num_neighbors)
        # tuple, each element in the tuple is a list of self.walk_length ndarrays, each with shape (batch_size, num_neighbors ** current_hop)
        dst_node_multi_hop_graphs = self.neighbor_sampler.get_multi_hop_neighbors(num_hops=self.walk_length,
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        assert (nodes_neighbor_times[:, :, 0] == node_interact_times.repeat(repeats=num_neighbors ** self.walk_length,
                                                                            axis=0).
...
class WalkEncoder(nn.Module):
    ...
    def __init__(self, input_dim: int, position_feat_dim: int, output_dim: int, num_walk_heads: int,
        ...
        self.output_dim = output_dim
        self.attention_dim = self.output_dim // 2
        self.num_walk_heads = num_walk_heads
        self.dropout = dropout
        ...
        self.feature_encoder = BiLSTMEncoder(input_dim=self.input_dim, hidden_dim=self.output_dim)
        ...
        self.projection_layers = nn.ModuleList([
            nn.Linear(in_features=self.feature_encoder.model_dim + self.position_encoder.model_dim,
...
class BiLSTMEncoder(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        self.bilstm_encoder = nn.LSTM(input_size=input_dim, hidden_size=self.hidden_dim_one_direction, batch_first=True,
                                      bidirectional=True)
