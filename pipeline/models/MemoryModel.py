...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.memory_dim = self.output_dim
        ...
        if self.model_name == 'PINT':
            self.matrix_memory = MatrixMemory(num_node=self.num_nodes, num_hop=num_hop, device=device)
        if self.model_name in ['TGN', 'PINT']:
        ...
        elif self.model_name == 'PINT':
            self.embedding_module = PINTEmbedding(node_raw_features=self.node_raw_features,
                                                  edge_raw_features=self.edge_raw_features,
                                                  neighbor_sampler=neighbor_sampler,
                                                  time_encoder=self.time_encoder,
                                                  node_feat_dim=self.node_feat_dim,
                                                  ...
                                                  output_dim=self.output_dim,
                                                  num_layers=self.num_layers,
                                                  matrix_memory=self.matrix_memory,
                                                  device=self.device,
                                                  beta=beta,
                                                  r_dim=num_hop + 1)
        else:
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        # get updated node memories and last updated timestamps
        # updated_node_memories, tensor, shape (2 * batch_size, memory_dim),
        # updated_node_last_updated_times, tensor, shape (2 * batch_size,),
        updated_node_memories, updated_node_last_updated_times = self.get_updated_memories(node_ids=node_ids)
        ...
        if self.model_name == 'JODIE':
            ...
            src_node_time_intervals = torch.from_numpy(node_interact_times).float().to(self.device) - \
                                      updated_node_last_updated_times[:len(src_node_ids)]
            ...
            node_embeddings = self.embedding_module.compute_node_temporal_embeddings(
                ...
                dst_node_ids=np.concatenate([dst_node_ids, src_node_ids]))
        else:
            raise ValueError(f'Not implemented error for model_name {self.model_name}!')
        ...
        if edges_are_positive:
            ...
            self.update_memories(node_ids=node_ids)
            ...
            if self.model_name == 'PINT':
                self.matrix_memory.update(src_node_ids=src_node_ids, dst_node_ids=dst_node_ids)
        ...
        if self.model_name == 'DyRep':
            src_node_embeddings = updated_node_memories[:len(src_node_ids)]
            dst_node_embeddings = updated_node_memories[len(src_node_ids):]
    ...
    def get_updated_memories(self, node_ids: np.ndarray):
        ...
        node_last_updated_times = self.memory_bank.get_node_last_updated_times(unique_node_ids=unique_node_ids)

        # aggregate messages for nodes that have stored messages
        ...
        # node_indexes, ndarray, shape (num_unique_node_ids,),  array to reconstruct original unique_node_ids from to_update_node_ids
        to_update_node_ids, unique_node_messages, unique_node_timestamps, node_indexes = self.message_aggregator.aggregate_messages(
            node_ids=unique_node_ids,
            node_raw_messages=self.memory_bank.node_raw_messages)

        # replace old memory with updated memory for to_update_node_ids with messages stored in previous batches (just for computation)
        ...
        node_last_updated_times[node_indexes] = torch.from_numpy(unique_node_timestamps).float().to(self.device)
    ...
    def update_memories(self, node_ids: np.ndarray):
        ...
        self.memory_updater.update_memories(unique_node_ids=to_update_node_ids,
                                            unique_node_messages=unique_node_messages,
                                            unique_node_timestamps=unique_node_timestamps)
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        # the original memory may be modifyied by in-place operation, so we clone the fetch memory
        # to ensure the backward process do not rely on the original memory
        ...
        # clone the new_src_node_raw_messages so that the store messages are not the view of new_src_node_raw_messages
        # by this way, when a node's message is released, the corresponding rows of the new_src_node_raw_messages will be released
        for i in range(len(src_node_ids)):
            new_node_raw_messages[src_node_ids[i]].append(
                (new_src_node_raw_messages[i].clone(), node_interact_times[i].copy()))
    ...
    def set_neighbor_sampler(self, neighbor_sampler: NeighborSampler):
        ...
        assert self.model_name in ['TGN', 'DyRep',
                                   'PINT'], f'Neighbor sampler is not defined in model {self.model_name}!'
...
class MessageAggregator(nn.Module):
    ...
    def aggregate_messages(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        # mimic the np.unique() to return the index of the to_update_node_ids in the origin node_ids
        node_indexes = []
...
class MatrixMemory(nn.Module):
    def __init__(self, num_node, num_hop, device):
        super(MatrixMemory, self).__init__()
        self.num_node = num_node
        self.num_hop = num_hop
        self.device = device
        self.matrix = nn.Parameter(torch.zeros((num_node, num_node, num_hop + 1)), requires_grad=False)
        self.P = nn.Parameter(torch.zeros(num_hop + 1, num_hop + 1), requires_grad=False)
        self.P.data[1:, :-1] = torch.eye(self.num_hop)
        self.reset_memory()

    def reset_memory(self):
        self.matrix.data[:, :, 0] = torch.eye(self.num_node)

    def update(self, src_node_ids, dst_node_ids):
        node_ids = torch.from_numpy(np.concatenate([src_node_ids, dst_node_ids])).to(self.device)
        node_message = torch.matmul(self.matrix[np.concatenate([dst_node_ids, src_node_ids])], self.P[None, :, :])
        self.matrix.scatter_add_(dim=0, index=node_ids[:, None, None].expand(node_message.shape), src=node_message)

    def get_memory(self, src_node_ids, dst_node_ids):
        matrix_memory = self.matrix[src_node_ids, dst_node_ids]
        # TODO: why divide the sum of temporal walk
        matrix_memory = matrix_memory / (torch.sum(matrix_memory, dim=1, keepdim=True) + 1e-4)
        return matrix_memory

    def backup_memory(self):
        return self.matrix.data.clone()

    def reload_memory(self, data):
        self.matrix.data = data.clone()
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, num_nodes: int, memory_dim: int):
        ...
        # a set of node ids, recoding nodes whose messages are modifies since last detach
    ...
    def get_memories(self, node_ids: np.ndarray):
        ...
        :return:
        """
        # the node memories may be modified by in-place operation, so we clone the fetched memory
        # to ensure the backward process do not rely on the original memory
    ...
    def backup_memory_bank(self):
        ...
        return self.node_memories.data.clone(), self.node_last_updated_times.data.clone(), cloned_node_raw_messages, self.dirty_message_nodes.copy()
    ...
    def reload_memory_bank(self, backup_memory_bank: tuple):
        ...
        self.node_memories.data, self.node_last_updated_times.data = backup_memory_bank[0].clone(), backup_memory_bank[
            1].clone()
        self.dirty_message_nodes = backup_memory_bank[3].copy()
    ...
    def detach_memory_bank(self):
        ...
        for node_id in self.dirty_message_nodes:
            node_raw_messages = self.node_raw_messages[node_id]
    ...
    def store_node_raw_messages(self, node_ids: np.ndarray, new_node_raw_messages: dict):
        ...
        self.dirty_message_nodes = self.dirty_message_nodes | set(node_ids.tolist())
    ...
    def get_node_last_updated_times(self, unique_node_ids: np.ndarray):
        ...
        :return:
        """
        # the last updated times may be modified by in-place operation, so we clone the fetched times
        # to ensure the backward process do not rely on the original last updated times
...
class TimeProjectionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_time_intervals: torch.Tensor):
        """
        compute node temporal embeddings using the embedding projection operation in JODIE
        :param node_memories: Tensor, shape (batch_size, memory_dim)
        :param node_ids: ndarray, shape (batch_size, )
        ...
        source_embeddings = self.dropout(
            node_memories * (1 + self.linear_layer(node_time_intervals.unsqueeze(dim=1))))
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def __init__(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor,
                 neighbor_sampler: NeighborSampler, time_encoder: TimeEncoder, node_feat_dim: int, edge_feat_dim: int,
                 time_feat_dim: int, output_dim: int,
        ...
        self.temporal_conv_layers = nn.ModuleList([MultiHeadAttention(node_feat_dim=self.output_dim,
                                                                      edge_feat_dim=self.edge_feat_dim,
                                                                      time_feat_dim=self.time_feat_dim,
                                                                      num_heads=self.num_heads,
        ...
        self.merge_layers = nn.ModuleList(
            [MergeLayer(input_dim1=self.temporal_conv_layers[-1].query_dim, input_dim2=self.output_dim,
                        hidden_dim=self.output_dim, output_dim=self.output_dim) for _ in
             range(num_layers)])

        assert self.node_feat_dim <= self.output_dim, "node_feat_dim should not be larger than output_dim!"
        # we need the projection layer to map node raw features to the same dimension of node memories (equals to self.output_dim)
        if self.node_feat_dim < self.output_dim:
            self.projection_layer = nn.Linear(in_features=self.node_feat_dim, out_features=self.output_dim, bias=True)
        else:
            self.projection_layer = None

    def compute_node_temporal_embeddings(self, get_updated_memories: Callable,
                                         node_ids: np.ndarray,
                                         node_interact_times: np.ndarray,
        ...
        :param get_updated_memories: a function whose input and output are node_ids and (update_memories,last_updated_times)
        ...
        if current_layer_num == 0:
            ...
            node_conv_features = self.compute_node_temporal_embeddings(get_updated_memories=get_updated_memories,
                                                                       node_ids=node_ids,
                                                                       node_interact_times=node_interact_times,
            ...
            neighbor_node_conv_features = self.compute_node_temporal_embeddings(
                get_updated_memories=get_updated_memories,
            ...
            # Tensor, output shape (batch_size, output_dim + time_feat_dim)
...
class PINTEmbedding(nn.Module):
    def __init__(self, node_raw_features, edge_raw_features, neighbor_sampler, time_encoder, num_layers,
                 node_feat_dim, edge_feat_dim, time_feat_dim, output_dim, matrix_memory: MatrixMemory, device, beta,
                 r_dim):
        super(PINTEmbedding, self).__init__()
        self.node_raw_features = node_raw_features
        self.edge_raw_features = edge_raw_features
        self.neighbor_sampler = neighbor_sampler
        self.time_encoder = time_encoder
        ...
        self.time_feat_dim = time_feat_dim
        self.output_dim = output_dim
        self.matrix_memory = matrix_memory
        self.r_dim = r_dim
        self.linear_1 = torch.nn.ModuleList([torch.nn.Linear(output_dim +
                                                             edge_feat_dim + 2 * r_dim, output_dim)
                                             for _ in range(num_layers)])
        self.linear_11 = torch.nn.ModuleList([torch.nn.Linear(output_dim, output_dim)
                                              for _ in range(num_layers)])
        self.linear_2 = torch.nn.ModuleList(
            [torch.nn.Linear(2 * output_dim + 2 * r_dim,
                             output_dim) for _ in range(num_layers)])

        self.linear_22 = torch.nn.ModuleList(
            [torch.nn.Linear(output_dim,
                             output_dim) for _ in range(num_layers)])
        self.alpha = 2
    ...
    def compute_node_temporal_embeddings(self, get_updated_memories: Callable,
                                         node_ids: np.ndarray,
                                         node_interact_times: np.ndarray,
                                         current_layer_num: int, num_neighbors: int, src_node_ids: np.ndarray,
        ...
        node_features, _ = get_updated_memories(node_ids)
        if self.projection_layer is None:
            node_features = node_features + self.node_raw_features[node_ids]
        else:
            node_features = node_features + self.projection_layer(self.node_raw_features[node_ids])
        ...
        if current_layer_num == 0:
            ...
            neighbor_embeddings = self.compute_node_temporal_embeddings(get_updated_memories=get_updated_memories,
                                                                        node_ids=neighbor_node_ids.flatten(),
                                                                        node_interact_times=neighbor_times.flatten(),
                                                                        current_layer_num=current_layer_num - 1,
                                                                        num_neighbors=num_neighbors,
                                                                        src_node_ids=np.repeat(src_node_ids,
                                                                                               num_neighbors),
                                                                        dst_node_ids=np.repeat(dst_node_ids,
                                                                                               num_neighbors))
            neighbor_embeddings = neighbor_embeddings.reshape(node_ids.shape[0], num_neighbors,
                                                              self.output_dim)
            neighbor_edge_features = self.edge_raw_features[torch.from_numpy(neighbor_edge_ids)]

            neighbors_features = torch.cat([neighbor_embeddings, neighbor_edge_features], dim=2)

            neighbor_delta_times = torch.from_numpy(node_interact_times[:, None] - neighbor_times).to(device=device,
                                                                                                      dtype=torch.float)
            mask = torch.from_numpy(neighbor_node_ids).to(device) != 0
            number_neighbors = torch.sum(mask, dim=1)[:, None, None]

            neighbor_embeddings = self.linear_1[current_layer_num - 1](torch.cat(
                [neighbors_features,
                 self.matrix_memory.get_memory(src_node_ids=np.repeat(src_node_ids, num_neighbors),
                                               dst_node_ids=neighbor_node_ids.reshape(-1)).
                 reshape(batch, num_neighbors, -1),
                 self.matrix_memory.get_memory(src_node_ids=np.repeat(dst_node_ids, num_neighbors),
                                               dst_node_ids=neighbor_node_ids.reshape(-1)).
                 reshape(batch, num_neighbors, -1)
                 ], dim=2))

            neighbor_embeddings = torch.relu(neighbor_embeddings)
            neighbor_embeddings = self.linear_11[current_layer_num - 1](neighbor_embeddings)

            # TODO: why divide the square of the number_neighbors
            neighbors_sum = neighbor_embeddings * (self.alpha ** (
                    -torch.relu(self.beta * neighbor_delta_times[:, :, None]) / torch.sqrt(number_neighbors + 1e-4)))

            neighbors_sum = torch.sum(neighbors_sum * mask[:, :, None], dim=1)

            node_embeddings = torch.cat(
                [neighbors_sum, node_features,
                 self.matrix_memory.get_memory(src_node_ids=src_node_ids, dst_node_ids=node_ids),
                 self.matrix_memory.get_memory(src_node_ids=dst_node_ids, dst_node_ids=node_ids),
                 ], dim=1)

            node_embeddings = self.linear_2[current_layer_num - 1](node_embeddings)
            node_embeddings = torch.relu(node_embeddings)
            node_embeddings = self.linear_22[current_layer_num - 1](node_embeddings)
