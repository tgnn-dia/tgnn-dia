...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, output_dim: int, channel_embedding_dim: int, patch_size: int = 1,
                 num_layers: int = 2, num_heads: int = 2, dropout: float = 0.1, max_input_sequence_length: int = 512,
                 device: str = 'cpu', use_cooccurrence: bool = True):
        """
        DyGFormer model.
        ...
        self.device = device
        # ablation: when False, the neighbour co-occurrence channel is zeroed
        # (architecture/param count unchanged, structural signal removed)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        # ablation: zero the co-occurrence channel (remove the explicit pairwise-structural signal)
        if not self.use_cooccurrence:
            src_padded_nodes_neighbor_co_occurrence_features = torch.zeros_like(src_padded_nodes_neighbor_co_occurrence_features)
            dst_padded_nodes_neighbor_co_occurrence_features = torch.zeros_like(dst_padded_nodes_neighbor_co_occurrence_features)
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: torch.Tensor,
                    ...
                    padded_nodes_neighbor_co_occurrence_features: torch.Tensor = None, patch_size: int = 1):
        """
