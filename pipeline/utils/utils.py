...
def get_logger(prefix, dataset_name, model_name):
    # set up logger
    ...
    fh = logging.FileHandler(
        f"./logs/{prefix}_link_{dataset_name}_{model_name}.log", mode="w")
...
def set_thread(num_thread):
    torch.set_num_threads(num_thread)


def set_random_seed(seed: int = 0, deterministic_alg=False):
    """
    set random seed
    ...
    torch.use_deterministic_algorithms(deterministic_alg)
    os.environ['PYTHONHASHSEED'] = str(seed)
    if deterministic_alg:
        os.environ['CUBLAS_WORKSPACE_CONFIG'] = ":16:8"


def get_neg_sample_per_edge_dict():
    return {
        'tgbl-wiki': 999,
        'tgbl-review': 100,
        'tgbl-coin': 20,
...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0,
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            sorted_per_node_neighbors = sorted(
                per_node_neighbors, key=lambda x: x[2])
    ...
    def compute_sampled_probabilities(self, node_neighbor_times: np.ndarray):
        ...
        exp_node_neighbor_times = np.exp(
            self.time_scaling_factor * node_neighbor_times)
        sampled_probabilities = exp_node_neighbor_times / \
            np.cumsum(exp_node_neighbor_times)
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_ids = np.zeros(
            (len(node_ids), num_neighbors)).astype(np.longlong)
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_neighbor_times = np.zeros(
            (len(node_ids), num_neighbors)).astype(np.float64)
        ...
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            ...
            if len(node_neighbor_ids) > 0:
                if self.sample_neighbor_strategy in ['uniform', 'time_interval_aware']:
                    ...
                    nodes_neighbor_ids[idx,
                                       :] = node_neighbor_ids[sampled_indices]
                    nodes_edge_ids[idx, :] = node_edge_ids[sampled_indices]
                    nodes_neighbor_times[idx,
                                         :] = node_neighbor_times[sampled_indices]
                    ...
                    nodes_neighbor_ids[idx,
                                       :] = nodes_neighbor_ids[idx, :][sorted_position]
                    nodes_edge_ids[idx, :] = nodes_edge_ids[idx,
                                                            :][sorted_position]
                    nodes_neighbor_times[idx,
                                         :] = nodes_neighbor_times[idx, :][sorted_position]
                    ...
                    nodes_neighbor_ids[idx, num_neighbors -
                                       len(node_neighbor_ids):] = node_neighbor_ids
                    nodes_edge_ids[idx, num_neighbors -
                                   len(node_edge_ids):] = node_edge_ids
                    nodes_neighbor_times[idx, num_neighbors -
                                         len(node_neighbor_times):] = node_neighbor_times
                else:
                    raise ValueError(
    ...
    def get_multi_hop_neighbors(self, num_hops: int, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        for hop in range(1, num_hops):
            ...
            nodes_neighbor_times = nodes_neighbor_times.reshape(
                len(node_ids), -1)
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None,
        ...
        if self.negative_sample_strategy != 'random' and self.negative_sample_strategy != 'new_random':
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        if self.negative_sample_strategy == 'random':
            ...
            negative_src_node_ids, negative_dst_node_ids = self.inductive_sample(size=size,
                                                                                 ...
                                                                                 current_batch_start_time=current_batch_start_time,
                                                                                 current_batch_end_time=current_batch_end_time)
        elif self.negative_sample_strategy == 'new_random':
            negative_src_node_ids, negative_dst_node_ids = self.new_random_sample(size=size,
                                                                                  batch_src_node_ids=batch_src_node_ids,
                                                                                  batch_dst_node_ids=batch_dst_node_ids)
        else:
            raise ValueError(
    ...
    def random_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray):
        """
        random sampling strategy, which is used by previous works
        :param size: int, number of sampled negative edges
        :return:
        """
        neg_num_per_edge = size // len(batch_src_node_ids)
        assert neg_num_per_edge * len(batch_src_node_ids) == size
        if self.seed is None:
        ...
        negative_src_node_ids = np.repeat(batch_src_node_ids, neg_num_per_edge)
        negative_dst_node_ids = self.unique_dst_node_ids[random_sample_edge_dst_node_indices]
        return negative_src_node_ids, negative_dst_node_ids

    def new_random_sample(self, size, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray):
        num_neg_per_edge = size // len(batch_src_node_ids)
        neg_src = np.repeat(batch_src_node_ids, num_neg_per_edge)
        neg_dst = []
        for m_dst in batch_dst_node_ids:
            m_unique_dst_node_ids = self.unique_dst_node_ids[self.unique_dst_node_ids != m_dst]
            assert len(m_unique_dst_node_ids) == len(
                self.unique_dst_node_ids) - 1
            neg_dst.append(np.random.choice(
                m_unique_dst_node_ids, num_neg_per_edge, replace=False))
        neg_dst = np.concatenate(neg_dst)
    ...
    def random_sample_with_collision_check(self, size: int, batch_src_node_ids: np.ndarray,
        ...
        return np.array([possible_random_edges[random_edge_idx][0] for random_edge_idx in random_edge_indices]), \
            np.array([possible_random_edges[random_edge_idx][1]
    ...
    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        unique_inductive_edges = historical_edges - \
            self.observed_edges - current_batch_edges
