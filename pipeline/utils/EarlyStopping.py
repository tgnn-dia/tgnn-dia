...
class EarlyStopping(object):
    ...
    def step(self, metrics: list, model: nn.Module, args: argparse.Namespace, epoch: int,
             aux_modules: dict = None):
        """
        ...
        if torch.all(torch.tensor(metrics_compare_results)):
            ...
            self.save_checkpoint(model, args, aux_modules=aux_modules)
            self.counter = 0
    ...
    def save_checkpoint(self, model: nn.Module, args: argparse.Namespace,
        ...
        :param model: nn.Module
        :param args:
        :param aux_modules: optional dict of name -> nn.Module whose state_dicts are also saved
        """
        ...
        if aux_modules:
            for name, module in aux_modules.items():
                save_object[name] = module.state_dict()
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None,
        ...
        :param aux_modules: optional dict of name -> nn.Module to restore from checkpoint
        """
        self.logger.info(f"load model {self.save_model_path}")
        save_object = torch.load(self.save_model_path,
        ...
        if aux_modules:
            for name, module in aux_modules.items():
                if name in save_object:
                    module.load_state_dict(save_object[name])
